...
always @ (posedge Clock_50) begin
	if (uut.SRAM_we_n == 1'b0) begin	//signal names within project (instantiated as uut) should match here, assuming names from experiment4a
		...
		if (uut.SRAM_address > 78600 && uut.SRAM_address < 146944) begin
...
initial begin: nasar_nathen
  integer i, j;
  #200;
  $write("First block of Y values is expected to be:\n");
  for (i=0; i<8; i++) begin
    for (j=4; j<8; j++)
      $write("%x ", SRAM_ARRAY[160*i+j][15:0]);
    $write("\n");
  end  
end

integer write_S_val_count = 0;

//always @ (posedge Clock_50) begin
//	if (uut.m2_unit.write_enable_b[1] == 1'b1) begin
//	   $write("%t: write S value %x in DP-RAM 1 at location %d (written values %d)\n", $realtime, uut.m2_unit.write_data_b[1], uut.m2_unit.address_b[1], ++write_S_val_count);
//  end
//end
